...
export class DuelScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.socketService.handleEvent(DuelEventsEnum.IKUJO, (res) => {
      ...
      duelist1.duelDeck = res.data.duelDeck.map(zc => new ZoneCard({
        card: zc.card,
        uid: zc.uid,
        ownerId: zc.ownerId,
      }));
    ...
    this.socketService.handleEvent(DuelEventsEnum.KOI, (res) => {
      ...
      duelist2.duelDeck = res.data.duelDeck.map(zc => new ZoneCard({
        card: zc.card,
        uid: zc.uid,
        ownerId: zc.ownerId,
      }));
  ...
  renderDuelistZones(duelist: Duelist) {
    ...
    const handZone = new DuelZone({
      name: ZonesEnum.HAND,
      uid: `${duelist.player.name}-${ZonesEnum.HAND}`,
      side: side,
      zoneCardSpacing: ZoneSpacingEnum.EVEN,
    }, this);
    ...
    height = cardHeight;
    yOrdinate -= directionFactor * height / 2;
    const manaZone = new DuelZone({
      name: ZonesEnum.MANA,
      uid: `${duelist.player.name}-${ZonesEnum.MANA}`,
      side: side,
      visibility: ZoneVisibilitiesEnum.PUBLIC,
      cardStraight: false,
      cardsCanTap: true,
      canSwitchCardSpacing: true,
    }, this);
    ...
    const hyperSpatialZone = new DuelZone({
      name: ZonesEnum.HYPERSPATIAL,
      uid: `${duelist.player.name}-${ZonesEnum.HYPERSPATIAL}`,
      side: side,
      visibility: ZoneVisibilitiesEnum.PROTECTED,
      cardStacked: true,
    }, this);
    hyperSpatialZone.render(xOrdinate, yOrdinate, cardWidth, height);

    xOrdinate -= directionFactor * cardWidth;
    const graveyardZone = new DuelZone({
      name: ZonesEnum.GRAVEYARD,
      uid: `${duelist.player.name}-${ZonesEnum.GRAVEYARD}`,
      side: side,
      visibility: ZoneVisibilitiesEnum.PUBLIC,
      cardStacked: true,
    }, this);
    ...
    xOrdinate -= directionFactor * cardWidth;
    const deckZone = new DuelZone({
      name: ZonesEnum.DECK,
      uid: `${duelist.player.name}-${ZonesEnum.DECK}`,
      side: side,
      visibility: ZoneVisibilitiesEnum.OFF,
      cardStacked: true,
    }, this);
    deckZone.render(xOrdinate, yOrdinate, cardWidth, height);

    xOrdinate -= directionFactor * (cardWidth / 2 + (gameStageWidth - cardWidth * 3) / 2);
    const shieldZone = new DuelZone({
      name: ZonesEnum.SHIELD,
      uid: `${duelist.player.name}-${ZonesEnum.SHIELD}`,
      side: side,
      visibility: ZoneVisibilitiesEnum.OFF,
      zoneCardSpacing: ZoneSpacingEnum.EVEN,
    }, this);
  ...
  renderBattleZone() {
    ...
    const battleZone = new DuelZone({
      name: ZonesEnum.BATTLE,
      uid: `${ZonesEnum.BATTLE}`,
      visibility: ZoneVisibilitiesEnum.PUBLIC,
      type: ZoneTypesEnum.SHARED,
      cardsCanTap: true,
    }, this);
