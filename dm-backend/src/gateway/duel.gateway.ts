...
export class DuelGateway implements OnGatewayInit {
  ...
  setUp(client: Socket, payload: SocketPayloadInterface): WsResponse<SocketPayloadInterface> {
    ...
    const allSet = this.persons[game.gameIdentifier].indexOf(game.inviter) !== -1 && this.persons[game.gameIdentifier].indexOf(game.challenger) !== -1;
    const response: SocketPayloadInterface = {gameRoom: room, data: {allSet: allSet}};
    return GatewayUtility.broadcastTo(client, room, DuelEventsEnum.ALL_SET, response);
  }

  @SubscribeMessage(DuelEventsEnum.IKUJO)
  ikujo(client: Socket, payload: SocketPayloadInterface): WsResponse<SocketPayloadInterface> {
    const room = payload.gameRoom;
    const data = payload.data;
    const cards: ZoneCardInterface[] = data.duelDeck;
    cards.forEach(each => each.uid = uuid.v4());
    const response: SocketPayloadInterface = {gameRoom: room, data: {msg: 'Ikuzo..!', duelDeck: data.duelDeck}};
    return GatewayUtility.broadcastTo(client, room, DuelEventsEnum.IKUJO, response);
  }

  @SubscribeMessage(DuelEventsEnum.KOI)
  koi(client: Socket, payload: SocketPayloadInterface): WsResponse<SocketPayloadInterface> {
    const room = payload.gameRoom;
    const data = payload.data;
    const cards: ZoneCardInterface[] = data.duelDeck;
    cards.forEach(each => each.uid = uuid.v4());
    const response: SocketPayloadInterface = {gameRoom: room, data: {msg: 'Koi..!', duelDeck: data.duelDeck}};
    return GatewayUtility.broadcastTo(client, room, DuelEventsEnum.KOI, response);
  }

  @SubscribeMessage(DuelEventsEnum.DROP_CARDS)
  cardDropped(client: Socket, payload: SocketPayloadInterface): WsResponse<SocketPayloadInterface> {
    const room = payload.gameRoom;
    const data = payload.data;
  ...
  tapUntapCard(client: Socket, payload: SocketPayloadInterface): WsResponse<SocketPayloadInterface> {
    const room = payload.gameRoom;
    const data = payload.data;
    const response: SocketPayloadInterface = {gameRoom: room, data: data};
